...
void get_row_col(int* row, int* col) {
	...
	if (!infile.is_open()) {
		std::cout << "ERROR: Cannot find full_camera_mapping.cfg, create with create_ascii.py" << std::endl;
		return;
	}
	...
	while (infile >> pixel >> slot >> asic >> ch >> tmpix >> r >> c >> sipmpix >> sp >> sp2 >> xpix >> ypix){
		row[pix] = r;
		col[pix] = c;
		++pix;
	}
