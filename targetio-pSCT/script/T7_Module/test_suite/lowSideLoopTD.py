...
def recordModule(module,eventsPerRun=std_eventsPerRun,my_ip=std_my_ip,tb_ip=std_tb_ip,board_ip=std_board_ip,board_def=std_board_def,asic_def=std_asic_def,Vped=std_Vped, outputDir="output/"):
	outRunList=[]
	outFileList=[]
	...
	for trimCounts in trimCountsList:
		...
		if saveData:
			...
			logfilename = "%s/lowSide_run%d.log" % (outdirname,runID)
