...
#load pedestal database
ped_database_name = outdir+'pedestal_database_run{}.h5'.format(ped_run)
ped = h5py.File(ped_database_name,'r')
...
for run_num in run_list:
    ...
    outfile = outdir+'calibrated_run{}.h5'.format(run_num)
    ...
    for modInd, Nmod in enumerate(modList):
	    ...
	    for Nchannel in xrange(16):
                ...
                #load proper branch of pedestal database
                ped_group = ped['Module{}/Asic{}/Channel{}'.format(Nmod,Nasic,Nchannel)]
		...
		print "Calibrating events for Module {}, Run {}, Asic {}, Channel {}".format(Nmod, run_num,Nasic, Nchannel)
                ...
                calibrated_samples = np.zeros((nEvents,nSamples))
		    ...
		    for sample in xrange(nSamples):
			    samples[ievt,sample] = int(wf.GetADC(sample))
                    ...
                    #get pedestal and perform subtraction on samples
                    try:
                        pedestal = ped_group['Block{}/Phase{}/pedestal'.format(blockNumber,blockPhase)]
                        calibrated_samples[ievt,:] = samples[ievt,:]-pedestal
                    except KeyError:
                        print 'ERROR Skipping event. Pedestal not found! block{} phase{}'.format(blockNumber,blockPhase)
                ...
                branch.create_dataset("calibrated_samples", data=calibrated_samples)                          
