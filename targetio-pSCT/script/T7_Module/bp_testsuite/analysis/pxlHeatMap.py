	...
	for asic in range(nasic):
		for ch in range(nch):
			for ievt in range(nEvents):
				...
				rawdata = reader.GetEventPacket(ievt, (4*modInd+asic)*(nch/chPerPacket)+(ch/chPerPacket))
				packet = target_driver.DataPacket()
				packet.Assign(rawdata, reader.GetPacketSize())
				header = target_driver.EventHeader()
				reader.GetEventHeader(ievt, header);
				wf = packet.GetWaveform(ch%chPerPacket)
				for sample in range(nSamples):
					ampl[ievt, modInd, asic, ch, sample] = wf.GetADC(sample)
				# avg the first 20 samples
				baseline=np.average(ampl[ievt,modInd,asic,ch,1:15])
				# get std dev of baseline
				stddev = np.std(ampl[ievt,modInd,asic,ch,1:15])
				# find the max of the waveform
				maxADC=ampl[ievt,modInd,asic,ch,20:].max()
				maxLoc=np.where(ampl[ievt,modInd,asic,ch,20:]==maxADC)[0][0]+20
...
for modInd in range(len(modList)):
	...
	c = ax.pcolor(physHeatArr[modInd,:,:], vmin=0, vmax=3500)
	# take off axes	
	ax.axis('off')
	ax.set_aspect('equal')
	...
	c4 = ax4.pcolor(physHeatArr[modInd,:,::-1], vmin=0, vmax=1000)
	# take off axes	
	ax4.axis('off')
...
heatFig.savefig("{}/{}_old_pxlHeatMap.png".format(savedir,runID))
...
heatReflectFig.savefig("{}/{}_old_pxlSkyHeatMap.png".format(savedir,runID))
"""
...
baseFig.savefig("{}/{}_old_pxlBaseMap.png".format(savedir,runID))
...
timeFig.savefig("{}/{}_old_pxlPeakTimeMap.png".format(savedir,runID))
...
rmsFig.savefig("{}/{}_old_pxlRMSbaselineMap.png".format(savedir,runID))
"""
