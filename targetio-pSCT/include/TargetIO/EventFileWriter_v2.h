...
class EventFileWriter_v2 : public EventFile {
 public:
  EventFileWriter_v2(const std::string& pFileName, uint16_t pNPacketsPerEvent, uint16_t pPacketSizeInByte);

  // For creating calibrated R1 file
  EventFileWriter_v2(const std::string& pR1FileName, CTA::TargetIO::EventFileReader* pR0Reader);
  virtual ~EventFileWriter_v2() { Close(); }
  ...
  void AddEvent(TargetDriver::RawEvent_v2* pRawEvent);
  void Close(bool pWriteDateEnd = true);
  void Flush();

  void StartWatchingBuffer(
      std::shared_ptr<TargetDriver::EventBuffer_v2> pEventBuffer);
  ...
  static std::shared_ptr<EventFileWriter_v2> MakeEventFileWriter(
      const CTA::TargetDriver::DataListener_v2& listener,
      const std::string& fname);

  void DoNotWrite() { fWriteRatio=0; }
  void SetWriteRatio (uint32_t pWriteRatio) { fWriteRatio=pWriteRatio; }

  void SetRTMWriteMinPeriod (uint64_t pRTMWriteMinPeriod) {
    fRTMWriteMinPeriod=pRTMWriteMinPeriod;
  }
 ...
 private:
  bool fWatching;
  uint32_t fWriteRatio;
  useconds_t fWatcherSleep;  //<! sleep between attempted reads and flushes
  uint32_t fEventsWritten;
  uint32_t fPacketsWritten;
  uint64_t fRTMWriteMinPeriod;
  uint32_t fRTMEventsWritten; // Events written to RealTimeMonitoring
  uint16_t fNPacketsPerEvent;
  uint16_t fPacketSizeInByte;
  std::shared_ptr<TargetDriver::EventBuffer_v2> fEventBuffer;
  std::string fFileName,fFilePath; //Stripped strings
  ...
  // Creates a dummy file with a single event for Real Time Monitoring.
  void RTMEventWriter(TargetDriver::RawEvent_v2* pRawEvent);
