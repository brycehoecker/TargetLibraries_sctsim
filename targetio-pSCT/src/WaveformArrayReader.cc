...
namespace CTA {
	namespace TargetIO {
		...
		WaveformArrayReader::WaveformArrayReader(const std::string &filepath, uint16_t skip_events, uint16_t skip_end_events, bool silent){
			...
			if (nModulesInFile > 1 && nModulesInFile <= 24) {
				...
				std::cerr << "WARNING: No case set up for files with N modules:" << nModulesInFile << std::endl;
			}
		...
		void WaveformArrayReader::GetR0Event(uint32_t event_index, uint16_t* waveforms, size_t n_pixels_w, 
												size_t n_samples_w, uint16_t& first_cell_id, bool& stale, 
												bool& missing_packets, uint64_t& tack, int64_t& cpu_s, int64_t& cpu_ns) {
			...
			if (n_pixels_w != fNPixels) {
				std::cerr << "WARNING: incorrect number of pixels in waveform array" << std::endl;
			}
			if (n_samples_w != fNSamples) {
				std::cerr << "WARNING: incorrect number of samples in waveform array" << std::endl;
			}
		...
		void WaveformArrayReader::GetR1Event(uint32_t event_index, float* waveforms, size_t n_pixels_w, size_t n_samples_w,
												uint16_t& first_cell_id, bool& stale, bool& missing_packets,uint64_t& tack,
												int64_t& cpu_s, int64_t& cpu_ns) {
			...
			for (uint16_t ipack = 0; ipack < fNPacketsPerEvent; ipack++) {
				...
				for (unsigned short iwav = 0; iwav < nWaveforms; iwav++) {
					...
					uint16_t pix = module*fNASICs*fNChannels + asic*fNChannels + channel;
					for (unsigned short isam = 0; isam < n_samples; isam++) {
						waveforms[pix*fNSamples+isam] =	((float) fWf.GetADC16bit(isam)/fScale)-fOffset;
					}
				}
			}
		}//void WaveformArrayReader::GetR1Event

		void WaveformArrayReader::GetBlock(uint32_t event_index) {
			uint32_t event_id = GetEventID(event_index);
