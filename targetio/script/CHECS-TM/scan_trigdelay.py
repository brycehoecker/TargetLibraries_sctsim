...
outdir = "/home/cta/luigi/CHECS-TM/data/20170502/trigdelay_scan4/"
outfileroot = "events_dly{0:04d}.tio"
my_ip = "192.168.12.1"
...
for dly in np.arange(0,4095,128):
    print "take data with a delay of {} ns".format(dly)
    listener = target_io.DataListener(kBufferDepth, kNPacketsPerEvent, kPacketSize)
    listener.AddDAQListener(my_ip)
    listener.StartListening()
    writer = target_io.EventFileWriter(outdir+outfileroot.format(dly), kNPacketsPerEvent, kPacketSize)
    buf = listener.GetEventBuffer()
    writer.StartWatchingBuffer(buf)

    ####start data taking
    module.WriteSetting("TriggerDelay", int(dly))
    time.sleep(1)
