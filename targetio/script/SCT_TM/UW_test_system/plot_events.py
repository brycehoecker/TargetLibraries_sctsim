	    ...
	    counter=0
            for ievt in range(nEvents):
                ...
                for sample in range(nSamples):
                    ...
                    avampl[asic, channel, sample]+=wf.GetADC(sample)*1.0/nEvents
        	        #if np.any(ampl == 0):
        	        #    print np.where(ampl == 0)

		blockNumber=(packet.GetRow() + packet.GetColumn()*8)
                blockPhase=(packet.GetBlockPhase() )
		#if(asic==2 and channel==0):
                #	if(np.any(ampl[ievt,asic, channel] > 750)):
                #		print ievt, "blockNumber", blockNumber, "blockPhase", blockPhase
		#		counter+=1		
...
#			ax1.set_ylim(550,1200)
	...
	try:
		...
		print "Directory couldn't be created. This could be ok and just mean that it already exists."	
        fig.savefig('%s/internal_run%dASIC%dw_ch%d.png' % (saveDir, runID, asic,c))
        #fig.savefig('run%d_asic%d_ped_change.pdf' % (runID, asic))
