...
#     j=0
#     print(i)
#     for event_index in tqdm(range(j*10,(j*10)+10,1)):
#         ret=reader.GetR0Event(event_index,waveforms)
#         print(ret)
#         firstcell=ret[0]
#         plt.plot(np.arange(firstcell,firstcell+448,1),waveforms[i],alpha=0.8)#,color="black")
...
#    n_events=50
firstcell=-100
event_count=np.min([n_events,20])#,n_events2])

for event_index in tqdm(range(1,event_count,1)):
    ret=reader.GetR0Event(event_index,waveforms)
    print(ret)
    ...
    #plt.plot(newwave)
    plt.plot(np.arange(firstcell,firstcell+n_samples,1),waveforms[0],alpha=0.8)#,color="black")
...
for i in range(10):
    print(i)
    for event_index in tqdm(range(i*10,(i*10)+10,1)):
        ret=reader.GetR0Event(event_index,waveforms)
        firstcell=ret[0]
        plt.plot(np.arange(firstcell,firstcell+448,1),waveforms[14],alpha=0.8)#,color="black")
    plt.ylabel('ADC count')
    plt.show()
