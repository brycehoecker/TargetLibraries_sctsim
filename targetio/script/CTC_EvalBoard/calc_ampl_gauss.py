...
for event_index in tqdm(range(0,n_events,1)):
    ...
    phase=ret[0]%32
    if ret[0]>16000 or ret[0]<200:
        continue
    #print("Phase",phase)
    wave=waveforms[0]
    f = signal.resample(wave, 1280)
    gauss_std = 15 #resample has 10 times the points -> 10 * std
    filtered_f = gaussian_filter1d(f, gauss_std)
    #for i in range(0,2):
    #   for j in range(128):
    #       allwaves.append(waveforms[i][j])
    fill=1
    peakpos_i=np.argmax(filtered_f)/10.
    peakval_i=np.max(filtered_f)
    if peakpos_i>49 or peakpos_i<46:
        fill=0
    #if peakval_i>200 or peakval_i<165:
    #    continue
    if fill==1:
        peakval.append(peakval_i)
        peakpos.append(peakpos_i)
    plot=0
    if plot==1:
    ...
    #    filtered_f = gaussian_filter1d(f, gauss_std)
        plt.plot(xfilt,filtered_f,label="filtered")
        #plt.show()
        plt.plot(xraw,wave,label="Channel 0")
        plt.title("200mV, 10ns FWHM Gauss, FirstCellID: {}".format(ret[0]))
...
peakpos_mean = np.mean(peakpos)
peakpos_std = np.std(peakpos)
plt.title("Mean: {:.2f}, STD: {:.2f} (ns)".format(peakpos_mean,peakpos_std))
plt.hist(peakpos,bins=np.arange(40,55,0.25))
plt.xlabel("Peak Position in ns")
plt.show()

peakval_mean = np.mean(peakval)
peakval_std = np.std(peakval)
plt.title("Mean: {:.2f}, STD: {:.2f} (mV)".format(peakval_mean,peakval_std))
plt.hist(peakval,bins=np.arange(120,250,1))
plt.xlabel("Peak Value in mV")
plt.show()
