...
for event_index in tqdm(range(0,n_events,1)):
    ...
    #print(waveforms[0:16,:])
    mean=np.mean(waveforms[0:16,:])
    std=np.std(waveforms[0:16,:])
    means.append(mean)
    stds.append(std)
    phase=ret[0]%32
    block=int(ret[0]/32)
    #print("Event",event_index,"Mean",mean,"STD",std)
    #
    #for i in range(0,2):
    #   for j in range(128):
#           allwaves.append(waveforms[i][j])
    ...
    if plot==1:
        ...
        x1=0
        if phase<20:
            plt.text(1,ymax-1,"{}".format(block))
        while True:
            ...
            if xvalue<116:
                plt.text(xvalue+1,ymax-1,"{}".format(block))
            x1+=1
        for k in range(1):
            wave=waveforms[k]
            if k==0:
                ...
                plt.plot(xfilt,filtered_f,label="Channel 0 filtered")
            plt.plot(xraw,wave,label="Channel {}".format(k),alpha=0.4)
            if np.max(wave)>50:
...
plt.ylabel("Pedestal corrected Mean in ADC counts")
plt.xlabel("Event Number")
plt.show()

plt.plot(stds,alpha=0.5)
plt.ylabel("Event Baseline STD in ADC counts")
plt.xlabel("Event Number")
plt.show()
