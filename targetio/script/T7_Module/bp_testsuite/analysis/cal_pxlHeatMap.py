...
#FIXME read in the calibrated waveforms from an hdf5 file
filename = "/home/ctauser/runFiles/calibrated_run{}.h5".format(runID)
...
get_dimensions = f['Module{}/Asic0/Channel0/calibrated_samples'.format(modList[0])]
...
for modInd, modNum in enumerate(modList):	
	for asic in range(nasic):
		for ch in range(nch):
			cal_samples = f['Module{}/Asic{}/Channel{}/calibrated_samples'.format(modNum,asic,ch)]
			ampl[modInd, asic, ch] = np.array(cal_samples)
...
for modInd, modNum in enumerate(modList):
        for asic in range(nasic):
                for ch in range(nch):
			#maxADC=reduced_ampl[modInd,asic,ch,35:].max()
			#maxLoc=np.where(reduced_ampl[modInd,asic,ch,35:]==maxADC)[0][0]+35
...
for modInd in range(len(modList)):
	...
	if full:
		...
		locReflect = tuple([loc[0],reflectLisst[loc[1]]])
...
#		physRMSArr[modInd,:,:]=np.rot90(physRMSArr[modInd,:,:],k=2)
	...
	ax.set_aspect('equal')
        ...
        c4 = ax4.pcolor(physHeatArr[modInd,:,::-1], vmin=0, vmax=1500)
        # take off axes
        ax4.axis('off')
...
cbar.set_label('Integrated Pulse', rotation=270,size=20,labelpad=24)
