...
for run_num in run_list:
    ...
    if os.path.isfile(filename)==True:
       ...
       # new_path = "{0}/target5and7data/runs_{1}0000_through_{1}9999/".format(os.environ['HOME'],str(run_num)[:-4]) #FIXME
        new_path = "{0}/test_suite/output/".format(os.environ['HOME'],str(run_num)[:-4])
    ...
    for Nasic in range(0,1):
	for Nchannel in range(3,4):
