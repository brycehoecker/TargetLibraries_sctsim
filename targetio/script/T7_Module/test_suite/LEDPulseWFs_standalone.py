...
std_kNPacketsPerEvent = 4 #One per channel
...
"""
def LEDPulseWaveforms(module,frequency,trigger,runDuration, groupList=[1], numBlock=2, deadtime=12500, triggerDly=580, my_ip=std_my_ip, kNPacketsPerEvent=std_kNPacketsPerEvent,thresh=thresh, outputDir="output/"):
...
#	time.sleep(0.1)
	...
	outFile = "output/run%d.fits" % (runID)
	outRunList.append(runID)
	outFileList.append(outFile)
	print "Writing to: %s" % outFile
#	numblock = 2
	#if multiple channels depend on FPGA settings
	kPacketSize = 2*(10 + 2*(numBlock*32+1)) #you can read it on wiresharks
	module.WriteSetting("MaxChannelsInPacket", 2)
	kNPacketsPerEvent=32
	...
	tempFileName = "output/temperature_log_run%d.txt" % (runID)
	...
	## 	logRegisters.logRegister(module, outdirname, runID)
		...
		if (trigger%2)==0: #for external trigger
			...
			module.WriteSetting("TACK_EnableTrigger",0x10000)
  			module.WriteSetting("ExtTriggerDirection", 1) # 0 output (hardsync), 1 input (external trigger)
...
#			tester.EnableExternalTrigger(True) #data is produced
			...
			readPeriod = 30
			if(runDuration>29):
				numberOfReadings=runDuration/readPeriod
				overhead = runDuration%readPeriod
				print "The number of readings: ", numberOfReadings
				for i in range(numberOfReadings):
					#module.WriteSetting("TACK_EnableTrigger",0x0)
					listener.StopListening()
					temp = temperature_utils.readTemperature(module)
