...
std::string kTMCounters[TM_NUM_COUNTERS] = {
    "CountGoodSyncTACKs",      "CountTACKsReceived",    "CountSyncErrors",
    "CountTACKRangeErrors",    "CountTACKParityErrors", "CountDataPackets",
    "CountDataPacketsEnabled", "CountForwardedPackets", "CountProcessedEvents"};
...
int main(int argc, char** argv) {
  ...
  CTA::TargetDriver::TargetModule tm("config/T5_M_FPGA_Firmware0xFEDA003D.def",
                                     "config/T5_ASIC.def", 0);
  ...
  if (simMode) {  // sim mode, generate some data and send it
    ...
    if (stat != TC_OK) {
      ...
      tester->PrintStatus();
      std::cout << "--> Tester FW version = " << tester->GetFirmwareVersion()
                << std::endl;
  ...
  std::cout << "Writing in a partial register and reading back" << std::endl;
  uint32_t readBack;
  std::cout << tm.ReturnCodeToString(
                   tm.ReadSetting("ExtTriggerDirection", readBack))
            << std::endl;
  std::cout << "Read 1: " << std::hex << readBack << std::endl;
  uint32_t writeIn = 0x1;
  std::cout << tm.ReturnCodeToString(
                   tm.WriteSetting("ExtTriggerDirection", writeIn))
            << std::endl;
  std::cout << tm.ReturnCodeToString(
                   tm.ReadSetting("ExtTriggerDirection", readBack))
            << std::endl;
  ...
  std::cout << "Read 1: " << std::hex << readBack << std::endl;
  writeIn = 0x1;
  std::cout << tm.ReturnCodeToString(tm.WriteSetting("Unused_0x4F_2", writeIn))
            << std::endl;
  std::cout << tm.ReturnCodeToString(tm.ReadSetting("Unused_0x4F_2", readBack))
            << std::endl;
  std::cout << "Write 1: " << std::hex << writeIn << std::endl;
  std::cout << "Read 2: " << std::hex << readBack << std::endl;
