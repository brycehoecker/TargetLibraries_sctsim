...
UDPClient::UDPClient(uint8_t pMaxAttempts, uint32_t pReceiveTimeout_ms,
                     uint32_t pDAQTimeout_ms)
...
int UDPClient::ConnectToServer(const std::string& pMyIP, uint16_t pMySourcePort,
  ...
  if (fSocket == -1) {
    std::cerr << "UDPClient::ConnectToServer: socket failed " << strerror(errno)
              << std::endl;
    return TC_ERR_COMM_FAILURE;
  }

  if (pSocketBufferSize > 0) {
    if (setsockopt(fSocket, SOL_SOCKET, SO_RCVBUF, &pSocketBufferSize,
                   sizeof(pSocketBufferSize)) != 0) {
  ...
  int ret = bind(fSocket, (struct sockaddr*)&sourceSocketAddress,
                 sizeof(sourceSocketAddress));

  if (ret != 0) {
    std::cerr << "UDPClient::ConnectToServer: bind failed " << strerror(errno)
              << ": port " << pMySourcePort << " ip " << pMyIP << std::endl;
    return TC_ERR_COMM_FAILURE;
  }
  std::cerr << "UDPClient::ConnectToServer: bind OK "
            << " port " << pMySourcePort << " ip " << pMyIP << std::endl;
  ...
  ret = connect(fSocket, (struct sockaddr*)&destinationSocketAddress,
                sizeof(destinationSocketAddress));

  if (ret != 0) {
    std::cerr << "UDPClient::ConnectToServer: connect failed "
              << strerror(errno) << std::endl;
    return TC_ERR_NO_RESPONSE;
  }

  if (fVerbose) {
    std::cout << "UDPClient set up to contact server on port " << pServerPort
              << " - will attempt all commands " << (int)fMaxAttempts
              << " times" << std::endl;
  }
...
int UDPClient::DiscardPacketsInTheSocketBuffer() {
  ...
  for (uint32_t i = 0; i < 100;
       ++i) {  // 100 is arbitrary, but should be enough
...
int UDPClient::Send(const void* buffer, uint32_t length) {
  ...
  if (fVerbose) {
    std::cout << "UDPClient::Send - sent " << bytes_written << " bytes"
              << std::endl;
  }
...
int UDPClient::AddDataListener(const std::string& pMyIP,
  ...
  if (pSocketBufferSize > 0) {
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &pSocketBufferSize,
                   sizeof(pSocketBufferSize)) != 0) {
      std::cerr << strerror(errno) << ".\n";
      return TC_ERR_COMM_FAILURE;
    }
  ...
  std::cout << "UDPClient::AddDataListener - port " << std::dec << pMyDataReceivePort
            << " and ip " << pMyIP << std::endl;
...
int UDPClient::SendAndReceive(const void* message, uint32_t length,
  ...
  while (attempts < fMaxAttempts) {
    ...
    // a fatal error
    if (stat < 0) {
      std::cerr << "Receive failed in: UDPClient::SendAndReceive, error code: "
                << stat << ": " << ReturnCodeToString(stat) << std::endl;
#ifdef WITH_MUTEX
      fSendAndReceiveMutex.unlock();
      // std::cout << "Error - Unlocked mutex " << fServerIPAddress <<
      // std::endl;
#endif
      return stat;
    }
...
#ifdef WITH_MUTEX
      fSendAndReceiveMutex.unlock();
      // std::cout << "OK - Unlocked mutex " << fServerIPAddress << " - " <<
      // resplength << std::endl;
#endif
    ...
    if (fVerbose) {
      std::cerr << "SendAndReceive attempt " << attempts + 1
                << "\t last attempt failed because of error code: " << stat
                << ": " << ReturnCodeToString(stat) << std::endl;
    }
    ++attempts;
  }
...
int UDPClient::GetDataPacket(void* tobefilled, uint32_t& bytes,
                             size_t maxbytes) {
  bytes = 0;
  ...
  for (PF_cit cit = fDAQPollList.cbegin(); cit != fDAQPollList.end(); ++cit) {
    if (cit->revents & POLLIN) {
      ...
      if (fVerbose) {
        std::cout << "UDPClient::GetDataPacket - received " << bytes
                  << " bytes from server " << ret << std::endl;
      }
