...
namespace CTA {
	namespace TargetDriver {
		...
		UDPBase::UDPBase() : fVerbose(false),
			fSocket(-1),
		...
		std::string UDPBase::ReturnCodeToString(int code) {
			switch (code) {
				...
				case TC_ERR_BAD_PACKET:
					return "Packet that was received was inconsistent with protocol expectations";
		...
		}//std::string UDPBase::ReturnCodeToString
		...
		int UDPBase::Receive(void* message, ssize_t& length, uint32_t max_length) {
			...
			length = recvfrom(fSocket, message, max_length, 0, (struct sockaddr*)&fSenderAddress, &len);

			fReceivedFromPort = ntohs(fSenderAddress.sin_port);

			if (fVerbose) {
				std::cout << "UDPBase::Receive - received " << length << " bytes from port " << fReceivedFromPort << std::endl;
			}
			if (length < 1) {
