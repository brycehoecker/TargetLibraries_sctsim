...
namespace CTA {
	namespace TargetDriver {
		UDPServer::UDPServer(const std::string& pMyIP, uint16_t pSlowControlListeningPort,
		...
		int UDPServer::Setup(const std::string& pMyIP, uint16_t pSlowControlListeningPort,
								uint16_t pDataDestPort, uint32_t receive_timout_ms) {
			...
			if (fVerbose) {
			std::cout << "UDPServer set up to listen on port " << pSlowControlListeningPort << std::endl;
			}
			return TC_OK;
		}

		int UDPServer::Send(const void* data, uint32_t length, int socket, uint16_t pDestPort) {
			if (fSenderAddress.sin_addr.s_addr == 0) {
				return TC_ERR_USER_ERROR;  // not contacted yet
			}

			fSenderAddress.sin_port = htons(pDestPort);
			ssize_t n = sendto(socket, data, length, 0, (struct sockaddr*)&fSenderAddress, sizeof(fSenderAddress));
			if (fVerbose) {
