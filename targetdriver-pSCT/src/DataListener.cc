...
DataListener::DataListener(uint32_t pBufferDepth, uint16_t pNPacketsPerEvent,
                           uint16_t pPacketSize, float pBuildTimeout,
                           uint32_t pCheckFreq)
    : UDPClient(),
      fNPacketsReceived(0),
      fNPacketProblems(0),
      fEventBuffer(std::make_shared<EventBuffer>(pBufferDepth,
                                                 pNPacketsPerEvent, pPacketSize,
                                                 pBuildTimeout, pCheckFreq)),
      fRunning(false),
      fDropPackets(false) {}

DataListener::~DataListener() { StopListening(); }
...
void DataListener::Listen() {
  ...
  dataPacket.Allocate(fEventBuffer->GetPacketSize());

  bool print_errors = true;

  //TACKs will be written out in case packets are dropped
  //std::ofstream tack_file;
  //tack_file.open("/home/cta/TACK_times.dat", std::ofstream::app);

  while (fRunning) {
    if (fNPacketProblems > 100 && print_errors) {
      std::cerr << "GetDataPacket: over 100 packet problems - suppressing "
                   "further print statements"
                << std::endl;
      print_errors = false;
    }
    uint32_t bytes = 0;
    ...
    int ret = GetDataPacket(reinterpret_cast<void*>(data), bytes,
                            dataPacket.GetPacketSize());
    ...
    switch (ret) {
      case TC_OK:
        ...
        break;
      case TC_TIME_OUT:
        // It's OK. Let's wait for a packet.
        continue;
      ...
      default:
        // Something wrong
        std::cerr << "GetDataPacket Problem: Unknown - SHOULD NEVER HAPPEN"
                  << std::endl;
        continue;
    }
    ...
    if (bytes != dataPacket.GetPacketSize()) {
      ...
      if (print_errors)
        std::cout << "DataListener::Listen() - Data packet is not the expected "
                     "size - was "
    ...
    if (!dataPacket.GetPacketID(packetID)) {
      ...
      // TODO(Akira): Error handling
      if (print_errors)
        std::cout << "DataListener::Listen() Could not calculate the packet ID "
                     "- status code: "
                  << dataPacket.GetStatus() << " - "
                  << dataPacket.GetStatusString() << std::endl;
      ++fNPacketProblems;
      continue;
    }
    ...
    /////////////////////////////////////
    if (!fDropPackets){
      fEventBuffer->AddNewPacket(dataPacket.GetData(), eventID, packetID,
                                 dataPacket.GetPacketSize());
      } else if (packetID == 0) {
...
*/
void DataListener::StartListening() {
  if (IsRunning()) {
    return;
  }
  ...
  std::cout << "DataListener::StartListening() - reseting counters - previous "
            ...
            << std::endl;
  fNPacketProblems = 0;
  fThread = std::thread(&DataListener::Listen, this);
}

/*!
@brief Stops the Listen method
*/
void DataListener::StopListening() {
  if (!IsRunning()) {
    return;
  }
