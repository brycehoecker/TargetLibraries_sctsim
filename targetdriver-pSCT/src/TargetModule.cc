...
namespace CTA {
	namespace TargetDriver {
		...
		TargetModule::TargetModule(uint16_t pModuleId): UDPClient(TM_MAX_ATTEMPTS, TM_RESPONSE_TIME_OUT, TM_DAQ_TIME_OUT),
		fTargetSettings("", ""), fTargetSettingsDefault("", ""), fModuleId(pModuleId), fClockPhase(0), fState(TM_STATE_UNDEFINED),
		fModuleType(kNonTM), fClientIP(""), fModuleIP("") {}

		TargetModule::TargetModule(const std::string& pFPGADef, const std::string& pASICDef, uint16_t pModuleId)
		: UDPClient(TM_MAX_ATTEMPTS, TM_RESPONSE_TIME_OUT, TM_DAQ_TIME_OUT), fTargetSettings(pFPGADef, pASICDef),
		fTargetSettingsDefault(pFPGADef, pASICDef),	fModuleId(pModuleId), fClockPhase(0), fState(TM_STATE_UNDEFINED),
		fModuleType(kNonTM), fClientIP(""),	fModuleIP("") {
			...
			} else if (fTargetSettings.fType == "TM_SCT") {
				fModuleType = kSCTModule;
			} else if (fTargetSettings.fType == "TM_SCT_BP") {
				fModuleType = kSCTModuleBP;
			} else {
				std::cerr
				<< "Unknown target module type defined firmware definition file:"
				<< std::endl;
		...
		void TargetModule::SetIPAddresses(const std::string& pClientIP, const std::string& pModuleIP) {
			fClientIP = pClientIP;
			fModuleIP = pModuleIP;
		}//void TargetModule::SetIPAddresses

		TargetModule::TargetModule(const std::string& pFPGADef, const std::string& pASICDef, const std::string& pTriggerASICDef, uint16_t pModuleId)
							: fTargetSettings(pFPGADef, pASICDef, pTriggerASICDef), fTargetSettingsDefault(pFPGADef, pASICDef, pTriggerASICDef),
							fModuleId(pModuleId), fClockPhase(0), fState(TM_STATE_UNDEFINED), fModuleType(kNonTM), fClientIP(""), fModuleIP("") {
			// Set Target module type
			if (fTargetSettings.fType == "TM_5") {
				fModuleType = kT5Module;
			} else if (fTargetSettings.fType == "TM_7") {
		...
		}//int TargetModule::AddDAQListener

		int TargetModule::ReconnectToServer(const std::string& pMyIP, uint16_t pMySourcePort, const std::string& pServerIP,	uint16_t pServerPort, int32_t pSocketBufferSize) {
			int stat = ConnectToServer(pMyIP, pMySourcePort, pServerIP, pServerPort, pSocketBufferSize);
			if (stat != TC_OK) {
			...
			if (stat != TC_OK) {
				std::cerr << "Fill RegisterMapFromFPGA failed, in " << "TargetModule::ReconnectToServer " << std::endl;
				return stat;
			}//if
			...
			stat = FillRegisterMapFromFPGA();
			if (stat != TC_OK) {
				std::cerr << "Fill UpdateFPGASettingMapFromRegisterMap failed, in " << "TargetModule::ReconnectToServer " << std::endl;
				return stat;
			}//if
			return stat;
		}//int TargetModule::ReconnectToServer
		...
		int TargetModule::GetPrimaryBoardID(uint64_t& pbid) {
			...
			std::cout << "GetPrimID: " << std::hex << " " << msw << " " << lsw << " " << ((msw & 0xFFFFFF) << 24) << " " << (lsw >> 8) << std::endl;
			return stat;
		}//int TargetModule::GetPrimaryBoardID
		...
		int TargetModule::GoToSafe() {
			...
			for (auto itF = fTargetSettingsDefault.fSettingMapFPGA.begin();
			itF != fTargetSettingsDefault.fSettingMapFPGA.end(); ++itF) {
			...
			if ((stat = WriteSetting("EventPortNumber", TM_DAQ_PORT)) != TC_OK)
			return stat;
			...
			fModuleType == kTCEval   || fModuleType == kTCModuleBP ||
			fModuleType == kTCModule|| fModuleType == kSCTModuleBP ||
			fModuleType == kSCTModule /* || fModuleType == kTCT5TEAEval*/) {
			// clang-format on
			if (fTargetSettings.fSettingMapFPGA.find("SetDataPort") !=
			...
			if ((stat = WriteSetting("HV_enable", 0)) != TC_OK) {
			return stat;
			}
			}
