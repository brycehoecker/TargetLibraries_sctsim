...
int TesterBoard::Init(const std::string& my_ip, const std::string& tb_ip) {
  ...
  int stat;
  if ((stat = ConnectToServer(my_ip, TESTER_COMMAND_PORT, tb_ip,
                              TESTER_BOARD_PORT)) != TC_OK)
    return stat;
  ...
  if (stat != TC_OK) {
    std::cout << "SetTriggerModeAndType: " << ReturnCodeToString(stat)
              << std::endl;
    return stat;
  }
  ...
  if ((stat = WriteRegisterPartially(0x10010, 31, 31, 0x0, 29, 29, 0x1)) !=
      TC_OK)
    return stat;
  if ((stat = WriteRegisterPartially(0x10010, 31, 31, 0x0, 29, 29, 0x0)) !=
      TC_OK)
    return stat;
...
void TesterBoard::EnableTrigger(uint8_t asic, bool b0, bool b1, bool b2,
                                bool b3) {
  if (asic > 3) {
...
void TesterBoard::EnableTrigger(uint8_t asic, uint8_t group, bool enable) {
  ...
  WriteRegisterPartially(0x10010, asic * 4 + group, asic * 4 + group,
                         enable ? 0x1 : 0x0);
}

void TesterBoard::EnableTriggerCounterContribution(uint8_t asic, uint8_t group,
                                                   bool enable) {
  if (asic > 3 || group > 3) {
    // TODO(Harm) throw RuntimeError("ASIC and group must be 3 or smaller");
  }  // if

  WriteRegisterPartially(0x10011, asic * 4 + group, asic * 4 + group,
                         enable ? 0x1 : 0x0);
}
...
*/
int TesterBoard::WriteRegisterPartially(uint32_t address, uint8_t msb,
                                        uint8_t lsb, uint32_t data) {
  if (msb > 31 || lsb > 31) {
...
int TesterBoard::WriteRegisterPartially(uint32_t address, uint8_t msb1,
  ...
  if (msb1 > 31 || lsb1 > 31 || msb2 > 31 || lsb2 > 31 || msb3 > 31 ||
      lsb3 > 31) {
    //    GERROR() << "Bits must be in between 0 and 31.";
    return TC_ERR_USER_ERROR;
  }  // if
