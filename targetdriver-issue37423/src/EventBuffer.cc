...
bool EventBuffer::AddNewPacket(const uint8_t* pData, uint32_t pEventID,
  ...
  if (found) {
	...
	std::cerr << "Buffer full. Failed to store packet " << pPacketID << " of event "
		  << pEventID << " - latest event ID was "
		  << (*fWriteIterator)->GetEventHeader().GetEventID()
		  << " - have now dropped - " << fPacketsDropped << " packets "
		  << std::endl;
      }
      return false;
    }
...
bool EventBuffer::AddNewPacket(const uint8_t* pData, uint32_t pEventID,
  ...
  for (std::size_t i = 0; i < n; ++i) {  // check all event slots in worst case
    ...
    if (evid == pEventID && event->IsBeingBuilt()) {
      ...
      if ((it == (fWriteIterator == fWriteIteratorNotInitialized
                      ? fBufferBegin
                      : std::next(fWriteIterator))) ||
  ...
  if ((fPacketsDropped == 1) || (fPacketsDropped % 10000) == 0) {
    std::cerr << "Failed to store packet " << pPacketID << " of event "
              << pEventID << " - latest event ID was "
              << (*fWriteIterator)->GetEventHeader().GetEventID()
              << " - have now dropped - " << fPacketsDropped << " packets "
              << std::endl;
...
void EventBuffer::AdvanceFinishedIterator(buffer_it pNextIterator) {
  ...
  if (fFinishedIterator == std::prev(pNextIterator) ||
      (fFinishedIterator == fBufferLast && pNextIterator == fBufferBegin)) {
    ++fFinishedIterator;
  } else if (fFinishedIterator == fNoFinishedEvents &&
             pNextIterator == fBufferBegin) {
...
float EventBuffer::GetEventRate() const {
  ...
  if ((fWriteIterator == fWriteIteratorNotInitialized) ||
      (fFinishedIterator == fNoFinishedEvents)) return 0.0;
  ...
  if (time_since>1.0) {
    std::cout << "No new events for " << time_since << " seconds. Finished iterator: " << 
      std::distance(fBufferBegin, fFinishedIterator) << " write iterator: " <<
      std::distance(fBufferBegin, fWriteIterator) << " incomplete: " << incomp <<
      " to be read: " << GetNumberToBeRead() << std::endl;
    return 0.0;
  }
  ...
  std::cout << std::dec << "Rate: " << rate << " Hz (from last " << evs << " events), Buffer Status: length " 
	    << buflen << " incomplete: " <<
    incomp << " to be read: " << GetNumberToBeRead() << std::endl;
...
void EventBuffer::Report(std::ostream& stream) const {
  ...
  stream << " failed waveform checks: " << fFailedWaveformChecks << " / "
         << fWaveformChecks;
