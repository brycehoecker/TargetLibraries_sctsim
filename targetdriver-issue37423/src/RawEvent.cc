...
RawEvent::RawEvent(uint16_t pNPacketsPerEvent, uint16_t pPacketSize)
    : fTimedOut(false) {
  Clear();
...
bool RawEvent::AddNewPacket(const uint8_t* data, uint16_t packetID,
                            uint16_t packetSize, bool checkflag) {
  ...
  if (filled) {
    ...
    if (checkflag) {
      ...
      if (!fWaveformCheckStatus)
        std::cerr << "RawEvent::FillPacket - Suspicious waveform - mean "
                  << mean << " rms " << rms << std::endl;
    }
  } else {
    std::cerr << "RawEvent::FillPacket - failed to fill packet ID: " << packetID
              << " size was " << packetSize << " expected "
              << packet->GetPacketSize() << " - event ID "
              << fEventHeader.GetEventID();
  }
